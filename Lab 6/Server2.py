...
class MyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
	...
	def do_HEAD(self):
		...
		if len(path_elements) == 4 and path_elements[-1] == '': # /worker/@id/
	...
	def do_GET(self):
		...
		if len(path_elements) == 3 and path_elements[-1] == '': # /worker/
		...
		elif len(path_elements) == 4 and path_elements[-1] == '': # /worker/@id/
			one = self.server.db.get_one(path_elements[2])
...
def run_server(host_name, port_number, file, message, server_class=MyServerClass, handler_class=MyHandler):
	...
	httpd.server_close()
	id(socket_listener_thread.event)
